/* $Header: /home/jcb/MahJong/newmj/RCS/gui.c,v 12.35 2025/09/07 15:44:20 jcb Exp $
 * gui.c
...
extern const char **fallbackpixmaps[];
...
# endif
  ...
  */
  ...
  if ( m->type == CMsgGame || m->type == CMsgSettlement ) {
    ...
    char buf[80];
    int i;
    static TileWind lastround = UnknownWind;
...
void open_connection(GtkWidget *w UNUSED, gpointer data, int oldfd, char *newaddr) {
  ...
  if ( mode < 3 || mode == 4 ) {
	...
	sscanf(newaddr,"%255[^:]",t);
      ...
      if ( snprintf(address,sizeof(address),"%s:%s",t,(mode == 4) ? newaddr : gtk_entry_get_text(GTK_ENTRY(openporttext))) < 0 ) {
	error("snprintf failed - this shouldn't happen");
      }
    }
    ...
    difficulty = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(opendifficultyspinbutton));
  ...
  if ( mode == 1 || mode == 2 ) {
	...
	strmcat(cmd,address,sizeof(cmd)-strlen(cmd));
	snprintf(cmd+strlen(cmd),100," --id %d ",i+2);
	strmcat(cmd," ",sizeof(cmd)-strlen(cmd));
	if ( robot_names[i][0] ) {
	  strmcat(cmd," --name ",sizeof(cmd)-strlen(cmd));
	  strmcat(cmd,robot_names[i],sizeof(cmd)-strlen(cmd));
	}
	if ( difficulty != 10 ) {
	  char s[100];
	  snprintf(s,100," --randomness %.1f ",(10-difficulty)/10.0);
	  strmcat(cmd,s,sizeof(cmd)-strlen(cmd));
	}
...
#   endif
  ...
  while ( rcf && ! feof(rcf) ) {
	...
	if ( strncmp(inptr,"Address",7) == 0 ) {
	  ...
	  inptr += 4;
	  sscanf(inptr," %255[^\r\n]",name);
	} else if ( strncmp(inptr,"Difficulty",10) == 0 ) {
	  inptr += 10;
	  sscanf(inptr," %d",&difficulty);
	} else if ( strncmp(inptr,"RobotName",9) == 0 ) {
	  int i;
	  char buf[128];
	  buf[127] = 0;
...
#     endif
  ...
  if ( update_groups & XmjrcOpen ) {
    ...
    /* don't save difficulty if it's 10 (default) */
    if ( difficulty < 10 ) {
      fprintf(ncf,"OpenDialog Difficulty %d%s",difficulty,nl);
    }
    for ( i = 0; i < 3; i++ ) {
      if ( robot_names[i][0] ) 
...
void create_display(void)
{
  ...
  */
  ...
  /* create the tile pixmaps */
  {
    ...
    /* Create a GC and color for drawing tile backgrounds. This is to work
   around a transparency/styling issue on macOS. By making the pixmap
   for face-down tiles opaque, we prevent the underlying button color
   (which is sometimes rendered incorrectly) from showing through. */
    gc = gdk_gc_new(topwindow->window);
    // colormap = gtk_widget_get_colormap(topwindow);
    colormap = gtk_widget_get_default_colormap();

    bg_color.red = 0xDDDD; bg_color.green = 0xDDDD; bg_color.blue = 0xDDDD;
...
#     endif
	...
	if ( pix == NULL ) {
	  ...
	  pix = gdk_pixbuf_new_from_xpm_data(fallbackpixmaps[j]);
	  numfailures++;
	}
	int h,w;
	...
	tilepixmaps[0][i] = gdk_pixmap_new(topwindow->window,w,h,-1);

	// gdk_draw_pixbuf(tilepixmaps[0][i],NULL,pix,
	gdk_draw_pixbuf(tilepixmaps[0][i],gc,pix,
			0,0,0,0,w,h,GDK_RGB_DITHER_NONE,0,0);

	tilepixmaps[2][i] = gdk_pixmap_new(topwindow->window,w,h,-1);

	pixu = gdk_pixbuf_rotate_simple(pix,180);

	// gdk_draw_pixbuf(tilepixmaps[2][i],NULL,pixu,
  gdk_draw_pixbuf(tilepixmaps[2][i],gc,pixu,
			0,0,0,0,w,h,GDK_RGB_DITHER_NONE,0,0);

	pixr = gdk_pixbuf_rotate_simple(pix,90);
	tilepixmaps[1][i] = gdk_pixmap_new(topwindow->window,h,w,-1);

  // gdk_draw_pixbuf(tilepixmaps[1][i],NULL,pixr,
  gdk_draw_pixbuf(tilepixmaps[1][i],gc,pixr,
			0,0,0,0,h,w,GDK_RGB_DITHER_NONE,0,0);

	pixl = gdk_pixbuf_rotate_simple(pix,270);
	tilepixmaps[3][i] = gdk_pixmap_new(topwindow->window,h,w,-1);

  // gdk_draw_pixbuf(tilepixmaps[3][i],NULL,pixl,
  gdk_draw_pixbuf(tilepixmaps[3][i],gc,pixl,

			0,0,0,0,h,w,GDK_RGB_DITHER_NONE,0,0);

	g_object_unref((GObject *)pix);
	g_object_unref((GObject *)pixr);
	g_object_unref((GObject *)pixl);
	g_object_unref((GObject *)pixu);
      }
    }

    g_object_unref((GObject *)gc);
	...
	tongpixmaps[0][i] =
	  gdk_pixmap_create_from_xpm_d(topwindow->window,
					 i>0 ? NULL : &tongmask,
				       NULL,(gchar **)fallbackpixmaps[101+i]);
	numfailures++;
      }
...
void destroy_display(void)
{
  ...
  if ( messagewindow ) gtk_widget_destroy(messagewindow); 
  messagewindow = NULL;
  if ( status_window ) gtk_widget_destroy(status_window); 
  status_window = NULL;
  if ( textwindow ) gtk_widget_destroy(textwindow); 
  textwindow = NULL;
